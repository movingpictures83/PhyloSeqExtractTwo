...
input <- function(inputfile) {
  ...
  filter1 <<- toString(parameters["FILTER1", 2])
  filter2 <<- toString(parameters["FILTER2", 2])
...
output <- function(outputfile) {
	...
	#print(sample_data(p0)[[column]])
	#print(filter1)
	#print(filter2)
	ps <- subset_samples(p0, (sample_data(p0)[[column]]%in% c(filter1,filter2)))
